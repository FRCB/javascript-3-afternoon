...
//Code Here
let totalPopulation = populations.reduce((total, element) => {
...
let ordersTotal = orders.map((element) => element.price + element.tax).reduce((total, element) => total + element);

//total = ordersTotal.reduce((accumulator, currentValue) => accumulator + currentValue);
...
let bobsTotal = purchases.filter((element) => element.owner === "Bob").map((element) => element.price).reduce((total, element) => total + element);
